...
| **R-30** | **Le chemin des signaux tiers n'a jamais pu produire un ordre.** Toute la chaîne de refus lit `setup.confidence_score` — `getattr(setup, "confidence_score", None) or 0` — alors que `ExternalSetup` ne posait que `confidence`. L'attribut manquant valait donc **0**, et un signal annonçant **92** était refusé en `below_confidence` contre un seuil de 60. Vérifié par exécution, pas par lecture. Conçu le 2026-08-26, route câblée le 28/08, **31 tests** — et inerte au septième portillon depuis le premier jour, en silence. `verdict_blockers` manquait également, bénin ici mais du même ordre. C'est la cinquième occurrence de la famille « écrit, testé en isolation, inerte en composition » : `external_signals` que rien n'appelait, `analyze_trend` sans un seul BEARISH sur 278 252 signaux, le contrôle de santé bouclant sur une liste vide (R-23), et ma propre déduplication passant 14 tests sans être branchée. **Corrigé le 2026-09-24**, avec `confidence` rendue obligatoire à l'entrée — un canal ne publie pas de score, c'est l'exploitant qui l'attribue, et cette décision doit être explicite | Risque | **E** | fait |
| **R-29** | **Le bridge ne distingue pas ses positions de celles d'un tiers sur le même compte.** `MAGIC_NUMBER` est **écrit** sur chaque ordre envoyé (4 sites) et **jamais lu** : `.magic` n'apparaît dans aucun chemin de lecture, et les trois `positions_get()` sont sans filtre. Conséquences tracées : `bridge.py:664` fait compter toute position étrangère dans le drawdown journalier et le plafond ; `:784` la fait consommer le budget de `_risque_engage_par_poche`, donc **bloquer les ordres du système** ; `:1868` la soumet à la boucle de surveillance — break-even, clôture partielle — donc **le bridge gère des trades qui ne sont pas les siens**. ⚠️ Ce n'est pas latent : R-13 établit que l'exploitant intervient à la main sur ce compte. Toute position ouverte hors du système est déjà traitée comme sienne. ⛔ Le correctif n'est pas « filtrer partout » — il est **différencié** : filtrer sur le magic pour *ce que je gère et ce que je compte comme mon exposition*, ne PAS filtrer pour *le risque de compte* (solde, perte journalière, marge), puisque l'argent, lui, est bien commun | Risque | **E** | 2 j |
| **R-28** | **La déduplication des signaux tiers ne protège pas contre deux canaux qui relaient le même appel.** L'unicité porte sur le couple `(source, external_id)` : elle empêche un fournisseur de rejouer sa file, pas deux fournisseurs d'émettre le même trade. Or les canaux de signaux se recopient — c'est la norme du milieu. Deux canaux suivis, un même appel « XAU sell 3900, SL 3920 » ⇒ **deux ordres, position doublée**, et rien ne le signale. Le commentaire du code énonce la limite sans la voir : « deux bots peuvent numéroter leurs signaux à partir de 1 sans s'écraser ». Latent aujourd'hui (`EXTERNAL_SIGNAL_TOKENS` est vide), il mord dès le **second** canal branché. Aggravé par R-1 : `correlation_guard` étant désactivé, rien ne garde non plus l'empilement de sources sur des instruments corrélés. Correctif : une déduplication **sémantique** — paire, sens, fenêtre de temps, niveaux proches — indépendante de la source | Risque | **E** | 2 j |
| **R-27** | **La suite de tests ne rend pas deux fois le même verdict.** Mesuré le 2026-09-24 : huit exécutions complètes du même commit, même ligne de commande, ordre figé (`-p no:randomly`). Sept rendent `4268 passed, 0 failed` ; une rend `4262 passed, 6 failed`. Les six concernés repassent au vert rejoués seuls. La cause reste **inconnue** : l'hypothèse d'un état partagé dans `trades.db` — fichier SQLite unique à la racine, écrit par les 4 268 tests — a été **testée et réfutée** (une base vierge donne le même résultat qu'une base chaude). ⛔ La conséquence ne dépend pas de la cause : **un « c'est vert » ne prouve plus rien**, puisque le même code produit les deux verdicts. Toute validation par compteur global est caduque ; seule tient une preuve ciblée — un test vérifié rouge sans le correctif et vert avec. Aggrave IT-1, qui ne recensait que « 3 tests dépendants de l'ordre » (§5.8) | IT | **E** | 3 j |
| **R-26** | **Le plafond de perte journalière ne couvre jamais un compte client.** `_destinations_reelles()` itère `DESTINATIONS`, qui **ne contient pas** la sentinelle `user:N`. Pour `user:2` la cascade de `silent_mode_active_for_destination` atteint donc « destination connue et NON réelle ⇒ jamais gelée » et rend `False`. Vérifié : `_destinations_reelles()` = `{admin_ibkr_us, admin_kraken, admin_kraken_spot, admin_kraken_stocks, admin_live}`, et `'user:2' not in reelles` avec `is_known('user:2') = True`. Un compte client peut donc saigner sans qu'aucun plafond n'engage. ⚠️ **Ce comportement est peut-être voulu** — la docstring dit « la démo perd de l'argent qui n'existe pas » — et il est correct **si et seulement si** les comptes Premium sont des comptes démo. Ce fait n'est pas établi : rien dans le code ne le garantit, et c'est le client qui branche son compte. ⛔ **Non corrigé le 2026-09-24 avec R-17/R-25** : ce chemin n'appelle pas `is_real_money`, et la réponse dépend d'un fait à trancher, pas d'un défaut de code | Risque | **E** | Décision |
| **R-25** | **Le verrou qui interdit l'argent réel aux signaux tiers ne couvre pas les comptes clients.** `resolve_destinations` écarte les destinations réelles dès que `setup.source` désigne un tiers — mais via `is_real_money()`, dont R-17 établit qu'elle rend `False` pour `user:N`. Vérifié : `is_real_money('admin_live') = True`, `is_real_money('user:2') = False` alors que `get('user:2')` rend bien un objet. Un signal de bot tiers est donc **barré du compte réel de l'exploitant et routé vers celui d'un client**. Le verrou est correctement placé (dans le résolveur, pas chez l'appelant) et sa docstring dit « n'atteint JAMAIS l'argent réel » : c'est vrai pour `admin_live`, faux pour un compte de tiers. Deuxième défaut de composition né du même `is_real_money` que R-17 ; combiné à C-1, il ouvre l'exécution des appels d'un tiers sur l'argent d'un autre tiers (§5.8) | Risque | **C** | 1 j |
| **R-24** | **Trois fonctions distinctes reposent sur le même lien, et tombent ensemble.** (1) L'accès RDP à la machine du bridge, (2) la route EC2→bridge et (3) le démarrage du bridge passent tous par Tailscale : le VPS est `100.74.160.72`, l'EC2 `100.103.107.75`, et `config/settings.py:293` confirme que le backend ne joint le bridge que par le tailnet. S'y ajoute que `launcher.ps1` est déclenché `AtLogOn user Administrator` — après un redémarrage, le bridge ne repart que si **un humain ouvre une session**, ce que la perte du tailnet empêche précisément. Une expiration de clé de nœud (par défaut ~180 jours chez Tailscale) supprime donc l'accès, l'exécution et la capacité de réparer, **d'un seul coup et sans changement de configuration**. Et R-23 garantit que rien ne le signale. Relevé le 2026-09-23 à l'occasion d'une perte d'accès RDP au VPS. Correctifs : ouverture de session automatique couplée à la fermeture du 3389 et à un accès par SSM (`deploy/bridge-windows-ec2.md`) ; **désactiver l'expiration de clé** sur les nœuds d'infrastructure ; surveiller `lastSeen` et `expires` par l'API Tailscale. Question ouverte : `ACTIVATION-PROCEDURE.md` parle du « VPS Cédric » — si cette machine n'appartient pas à l'exploitant, l'exécution en argent réel dépend d'un hôte qu'il ne contrôle pas, ce qui touche C-1 et IT-2 | IT | **E** | 1 j |
