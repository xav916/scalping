...
| **R-24** | **Le démarrage du bridge est lié à une ouverture de session interactive.** `launcher.ps1` est déclenché `AtLogOn user Administrator` : après un redémarrage, le bridge ne repart que si un humain ouvre une session RDP. Perdre RDP et perdre l'exécution sont donc le **même incident**, et R-23 garantit que rien ne le signale. Relevé le 2026-09-23 à l'occasion d'une perte d'accès RDP au VPS. Correctif : ouverture de session automatique, couplée à la fermeture du port 3389 et à un accès par SSM — les deux vont ensemble (`deploy/bridge-windows-ec2.md`) | IT | **E** | 1 j |
