...
1. **Pas de modif sur le live V1** entre maintenant et le gate. Toute modif pollue les expériences. Le live continue de tourner avec ses filtres anti-saigne (cf `project_scalping_algo_tuning_v1.md`).
   ...
   - **EXCEPTION 2026-04-25 ~17h UTC** : déploiement Phase 4 shadow log autorisé explicitement par l'utilisateur. Le déploiement inclut Bug #1 (b750fa5) puisqu'il est merged sur main, ainsi que tous les modules shadow (commit 173cad5 et antérieurs). Justification : la Phase 4 nécessite par construction un déploiement live pour observer les setups V2_CORE_LONG en réel. L'addition est read-only (try/except autour du shadow log, pas de modif scoring V1 ni de l'auto-exec V1).
